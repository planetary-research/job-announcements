...
def edit(slug):
    ...
    if request.method == "POST":
        if request.form.get("mode") == "edit_job":
            ...
            edit_job.country = escape(request.form["country"]).strip()
            edit_job.city = escape(request.form["city"]).strip()
            edit_job.work_arrangement = escape(request.form["work_arrangement"]).strip()
            edit_job.official_announcement_url = request.form["official_announcement_url"].strip()
            edit_job.duration = escape(request.form["duration"]).strip()
            edit_job.can_extend = escape(request.form["can_extend"]).strip()
            edit_job.salary = escape(request.form["salary"]).strip()
            edit_job.number_positions = escape(request.form["number_positions"]).strip()
            edit_job.reference_code = escape(request.form["reference_code"]).strip()
            edit_job.inquiries_name = escape(request.form["inquiries_name"]).strip()
            edit_job.inquiries_email = escape(request.form["inquiries_email"]).strip()
            edit_job.start_date_string = escape(request.form["start_date_string"]).strip()
        ...
        if request.form.get("mode") == "change_owner":
            ...
            # All good
                ...
                edit_job.owner_orcid = user_id
                edit_job.owner_name = orcid_name
                db.session.commit()

                base_data["redirect_alerts"] = {
                    "success": f"Job announcement owner was changed to {user_id} ({orcid_name}).",
                    "danger": None,
                    "info": None,
